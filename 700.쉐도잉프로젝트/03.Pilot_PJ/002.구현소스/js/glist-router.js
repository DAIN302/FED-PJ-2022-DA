...
let Glist = {
    ...
    <section>
        ...
        <!-- 상품 리스트 박스 -->
        <div class="grid">
            <div v-for="(v,i) in $store.state.gdata" v-if="v.cat==$store.state.selnm[0]||v.cat==$store.state.selnm[1]||v.cat==$store.state.selnm[2]" @click="$store.commit('setBtn')">
...
let More = {
    ...
    <section>
        <!-- 상품 리스트 박스 -->
        <div class="grid">
            <div v-for="(v,i) in $store.state.gdata" v-if="v.idx>=1 && v.idx<=5+$store.state.mnum">
                <router-link v-bind:to="{name:'det', params:{id:v.idx, list:$route.path}}">
                    [{{v.idx}}]
                    <img v-bind:src="'./images/goods/'+v.cat+'/'+v.ginfo[0]+'.png'" alt="dress" />
                    <aside>
    ...
    <div id="bgbx">
        ...
        <div id="imbx">
            <div class="inx">
                <!-- 큰 이미지 -->
                <section class="gimg">
                    <!-- $store.state.gdata[$route.params.id-1].cat :카테고리명(men/women/style) -->
                    <!-- $store.state.gdata[$route.params.id-1].ginfo[0] : 이미지명 -->
