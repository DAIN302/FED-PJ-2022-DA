...
/***************************************************************
    [ VueX Store - 뷰엑스 스토어 ]
    1. 데이터와 데이터상태를 한번에 관리하는 확장 라이브러리
    2. 스토어에서 관리되는 데이터는 리액티브데이터
       (reactive data :양방향 동기화 데이터) 
    3. 컴포넌트 구조 상태와 상관없이 사용하는 곳에서 변경한 내용이
       동기화되어 자연스런 업데이트가 이루어짐

    [ VueX Store 기본구조 ]   
    1. state(상태) : 변수선언 및 할당구역
       => 뷰인스턴스에서 data와 유사   
    2. mutations(변형) : 변수의 데이터를 변경하는 메서드 구역
       => 뷰인스턴스에서 methods와 유사 
    3. actions(실행) : 비동기 처리 메서드 구역  (백엔드 API에서 받은 내용 여기서 처리) 
    ...
    [ VueX Store 버전 이슈 ]
    - 일반적으로 뷰JS 버전이 2.x이면 vuex는 3.x버전을 사용
    (예컨데 최신 뷰는 3.x 이므로 vuex를 사용하려면 4.x 사용)
    -> 메이저버전 숫자가 뷰보다 하나큼
    (주의 : webpack 템플릿을 사용한 SPA 구축환경에서 이 버전이 맞지 않으면 
        뷰엑스 스토어에서 에러 발생)

    - 재밌는게 있음
    내보낼 때 const store = 뷰엑스 스토어 라고 해서 store가 호출명이 아님
    export default new Vuex.Store()
    위처럼 이름없이 내보내도 store 라는 이름으로 나간다
    즉, store는 정해진 이름   
