...
const hcode = {
    ...
    /*
        ...
        <span v-bind:class="'del'"></span>
        클래스명이 변수에 있으면 변수 쓰고 직접 문자로 넣으려면 따옴표 처리
        
        2. 조건에 의한 클래스 바인드
        <요소 v-bind:class={클래스명:조건}>
        <span v-bind:class="{del:haha==3}"></span>
        - 클래스의 값으로 객체를 설정하고 객체 속성명에 클래스명을 객체값에
        조건식을 쓰면 해당 조건에서만 클래스가 적용됨

        참고) 조건이 많을 경우 또는 여러군데 사용될 경우 조건식의 결과를 리턴하는
        메서드를 만들어 사용
        예) <span v-bind:class="{del:coodiRet()}"></span>
        ->>> 해당 컴포넌트 methods :{} 처리 구역에 리턴메서드 생성
        methods : {
            codiRet(){
                return this.haha==3 || this.haha==5 || .....
            }
        }
